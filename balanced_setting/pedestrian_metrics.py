...
def get_pedestrian_metrics(gt_label, preds_probs, threshold=0.5, index=None, cfg=None, balance_mask=None):
    """
    ...
    if balance_mask is None:
        ...
        # label metrics
        # TP + FN
        gt_pos = np.sum((gt_label == 1)*balance_mask, axis=0).astype(float)
        # TN + FP
        gt_neg = np.sum((gt_label == 0)*balance_mask, axis=0).astype(float)
        # TP
        true_pos = np.sum((gt_label == 1) * (pred_label == 1)*balance_mask, axis=0).astype(float)
        # TN
        true_neg = np.sum((gt_label == 0) * (pred_label == 0)*balance_mask, axis=0).astype(float)
        # FP
        false_pos = np.sum(((gt_label == 0) * (pred_label == 1))*balance_mask, axis=0).astype(float)
        # FN
        false_neg = np.sum(((gt_label == 1) * (pred_label == 0))*balance_mask, axis=0).astype(float)
        ...
        ################
        # instance metrics
        gt_pos = np.sum((gt_label == 1)*balance_mask, axis=1).astype(float)
        true_pos = np.sum((pred_label == 1)*balance_mask, axis=1).astype(float)
        # true positive
        intersect_pos = np.sum((gt_label == 1) * (pred_label == 1)*balance_mask, axis=1).astype(float)
        # IOU
        union_pos = np.sum(((gt_label == 1)*balance_mask + (pred_label == 1)*balance_mask), axis=1).astype(float)
